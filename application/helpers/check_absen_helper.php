...
function check_jam($jam, $status, $raw = false)
{
    if ($jam) {
        ...
        $CI = &get_instance();
        $jam_masuk = $CI->db->query("SELECT * FROM jam_kerja WHERE jadwal_kerja = '$status'")->row();

        $jam_kerja = $CI->db->get('jam_kerja')->result();

            if ($jam > $jam_masuk->dispensasi) {
                if ($raw) {
                    return [
                        // 'status' => 'telat',
                        'text' => $jam
                    ...
                    return "<span class='badge badge-success'>" . $jam . " Telat</span>";
                }
            }  else {
                if ($raw) {
                    return [
